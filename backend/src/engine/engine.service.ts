...
export class EngineService {
  ...
  async syncSource(source: string) {
    ...
    const companies = await this.companyService.findByATS(source);
  ...
  async sync() {
    ...
    try {
      for (const adapter of this.registry.getAdapters()) {
        const companies = await this.companyService.findByATS(adapter.source);

        this.logger.log('--------------------------------');
        this.logger.log(`Adapter : ${adapter.source}`);
        this.logger.log(`Companies : ${companies.length}`);
