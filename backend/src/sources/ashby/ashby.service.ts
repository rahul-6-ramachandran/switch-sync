...
export class AshbyService extends BaseAdapter {
  ...
  protected readonly logger = new Logger(AshbyService.name);
  ...
  protected async syncCompany(company: Company): Promise<void> {
    try {
      const response = await this.http.post<AshbyApiResponse>(
      ...
      const jobs = response.data?.jobBoard?.jobs ?? [];

      let synced = 0;

      for (const job of jobs) {
        const location = job.location?.locationName ?? undefined;
        ...
        await this.jobsService.upsertJob({
          ...
          remoteStatus: job.isRemote ?? isRemoteJob(location),
      ...
      this.logger.log(`${company.name}: ${synced}/${jobs.length} jobs synced`);
    } catch (error) {
      this.logger.error(`Failed syncing ${company.name}`, error);
    }
  }
}
